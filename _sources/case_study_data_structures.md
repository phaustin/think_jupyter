...
Chapter [22](#algorithms). But you don’t have to read it before you go
...
Section [10.1](#wordlist)) and then print all the words in the book
that are not in the word list. How many of them are typos? How many of
them are common words that <span>*should*</span> be in the word list,
...
The function <span>`random`</span> returns a random float between 0.0
and 1.0 (including 0.0 but not 1.0). Each time you call
<span>`random`</span>, you get the next number in a long series. To see
...
The function <span>`randint`</span> takes parameters <span>`low`</span>
and <span>`high`</span> and returns an integer between
<span>`low`</span> and <span>`high`</span> (including both).
...
defined in Section [12.2](#histogram) and returns a random value from
...
a time to `process_line`. The histogram <span>`hist`</span> is being
...
I use the keyword argument <span>`sep`</span> to tell
<span>`print`</span> to use a tab character as a “separator”, rather
than a space, so the second column is lined up. Here are the results
from <span>*Emma*</span>:
...
This code can be simplified using the <span>`key`</span> parameter of
the <span>`sort`</span> function. If you are curious, you can read about
it at <https://wiki.python.org/moin/HowTo/Sorting>.
...
<span>`num`</span> gets the value of the argument instead. In other
words, the optional argument <span>**overrides**</span> the default
value.
...
<span>`d2`</span> and returns a new dictionary that contains all the
keys from <span>`d1`</span> that are not in <span>`d2`</span>. Since we
...
To find the words in the book that are not in <span>`words.txt`</span>,
we can use `process_file` to build a histogram for
<span>`words.txt`</span>, and then subtract:
...
Section [20.5](#sets), or read the documentation at
...
The expression <span>`[word] * freq`</span> creates a list with
<span>`freq`</span> copies of the string <span>`word`</span>. The
<span>`extend`</span> method is similar to <span>`append`</span> except
...
2.  Build a list that contains the cumulative sum of the word
    frequencies (see Exercise [\[cumulative\]](#cumulative)). The last
...
3.  Choose a random number from 1 to \(n\). Use a bisection search (See
    Exercise [\[bisection\]](#bisection)) to find the index where the
    random number would be inserted in the cumulative sum.

4.  Use the index to find the corresponding word in the word list.

<span id="randhist" label="randhist">\[randhist\]</span>
...
<span>`shift`</span> takes a tuple of words, <span>`prefix`</span>, and
a string, <span>`word`</span>, and forms a new tuple that has all the
words in <span>`prefix`</span> except the first, and <span>`word`</span>
added to the end.
...
like the <span>`profile`</span> module, that can identify the places in
a program that take the most time.
...
\[\log f = \log c - s \log r\] So if you plot log \(f\) versus log
\(r\), you should get a straight line with slope \(-s\) and intercept
log \(c\).
...
Anaconda, you already have <span>`matplotlib`</span>; otherwise you
